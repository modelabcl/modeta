defmodule Modeta.SchemaCache do
  ...
  # 5 minutes TTL for cached schemas
  ...
  def init(_opts) do
    ...
    :ets.new(@table_name, [
      ...
      :public,
      # Use atom name instead of reference
      :named_table,
  ...
  defp fetch_and_cache_schema(group_name, collection_name) do
    case fetch_schema_from_database(group_name, collection_name) do
      {:ok, schema} ->
        ...
        Logger.debug(
          "Cached schema for #{group_name}.#{collection_name}: #{length(schema)} columns"
        )
      ...
      error ->
        Logger.warning(
          "Failed to fetch schema for #{group_name}.#{collection_name}: #{inspect(error)}"
        )
    ...
    try do
      ...
      Enum.each(all_collections, fn {group_name, collection_name} ->
        case get_schema(group_name, collection_name) do
          {:ok, schema} ->
            Logger.debug(
              "✓ Warmed cache for #{group_name}.#{collection_name} (#{length(schema)} columns)"
            )

          {:error, reason} ->
            Logger.warning(
              "✗ Failed to warm cache for #{group_name}.#{collection_name}: #{inspect(reason)}"
            )
        end

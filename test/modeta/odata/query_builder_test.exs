defmodule Modeta.OData.QueryBuilderTest do
  ...
  describe "build_query_with_options/9" do
    test "applies default pagination when no options provided" do
      ...
      result = QueryBuilder.build_query_with_options(
        base_query, "sales_test", "purchases", 
    ...
    test "handles collection not found gracefully" do
      ...
      result = QueryBuilder.build_query_with_options(
        base_query, "non_existent_group", "non_existent", 
        nil, nil, nil, nil, nil, nil
      )
  ...
  describe "apply_orderby_to_query/2" do
    ...
    test "applies multiple column ordering" do
      ...
      expected = "SELECT * FROM (#{base_query}) AS ordered_data ORDER BY country DESC, name ASC, age ASC"
      assert result == expected
    end
    ...
    test "ignores invalid column names" do
      ...
      expected = "SELECT * FROM (#{base_query}) AS ordered_data ORDER BY valid_column ASC, another_valid ASC"
      assert result == expected
    end
    ...
    test "handles qualified column names" do
      ...
      expected = "SELECT * FROM (#{base_query}) AS ordered_data ORDER BY customers.name ASC, table.column_name DESC"
      assert result == expected
    end
  end
  ...
  describe "integration with OData filter" do
    test "preserves filter application order" do
      ...
      result = QueryBuilder.build_query_with_options(
        base_query, "non_existent", "non_existent",
        "name eq 'John'", nil, "name", nil, nil, "5"
      )

defmodule Modeta.OData.QueryBuilderTest do
  ...
  describe "apply_pagination_to_query/3" do
    test "applies default pagination when no params provided" do
      ...
      # Should use default page size (1000) + 1 for pagination detection and skip 0
      expected = "SELECT * FROM (#{base_query}) AS paginated_data LIMIT 1001 OFFSET 0"
      assert result == expected
    end
    ...
    test "handles partial integer parsing as invalid" do
      ...
      # Since both have remaining chars, they default to 0 and 1000 (+ 1 for pagination detection)
      expected = "SELECT * FROM (#{base_query}) AS paginated_data LIMIT 1001 OFFSET 0"
      assert result == expected
    end
  end
  ...
  describe "integration with OData filter" do
    test "preserves filter application order" do
      ...
      # Should have filter, then select, then pagination (+ 1 for pagination detection)
  ...
  describe "query building pipeline" do
    test "applies operations in correct order without collection config" do
      ...
      # Let's verify the structure contains expected parts (+ 1 for pagination detection)
